...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .profile-header { padding: 10px 12px; border-bottom: 1px solid #fce7f3; margin-bottom: 8px; }
        .profile-header strong { display: block; font-size: 14px; color: #1f2937; }
        .profile-header span { display: block; font-size: 12px; color: #6b7280; margin-top: 4px; word-break: break-word; }

        .profile-dropdown a, .dropdown-logout {
            display: block; width: 100%; text-align: left; padding: 10px 12px;
            border: none; background: transparent; text-decoration: none;
            color: #374151; font-size: 14px; border-radius: 10px; cursor: pointer;
            transition: background .2s ease, color .2s ease;
        }

        .profile-dropdown a:hover, .dropdown-logout:hover { background: #fdf2f8; color: #f43f5e; }

        .sidebar {
            position: fixed; top: var(--topbar-height); left: 0;
            width: var(--sidebar-width); height: calc(100vh - var(--topbar-height));
            background: var(--pink-sidebar); border-right: 1px solid #efd5e1;
            padding: 22px 12px; display: flex; flex-direction: column; justify-content: space-between;
        }

        .sidebar-title { color: #c06a8d; font-size: .74rem; text-transform: uppercase; letter-spacing: .14em; margin-bottom: 14px; padding: 0 8px; font-weight: 700; }
        .nav-menu { display: flex; flex-direction: column; gap: 8px; }
        
        .nav-link {
            display: flex; align-items: center; justify-content: space-between; gap: 10px;
            text-decoration: none; color: #b04877; padding: 12px 14px; border-radius: 14px;
            font-weight: 600; transition: .2s ease;
        }

        .nav-link-left { display: flex; align-items: center; gap: 10px; }
        .nav-link:hover { background: rgba(228, 77, 141, 0.12); }
        
        .nav-link.active {
            background: linear-gradient(90deg, rgb(244, 63, 110) 0%, rgb(236, 72, 153) 50%, rgb(219, 39, 119) 100%);
            color: #fff; box-shadow: 0 10px 24px rgba(228, 77, 141, 0.24);
        }

        .nav-icon { width: 18px; text-align: center; font-size: 14px; }

        .content { margin-left: var(--sidebar-width); padding: 84px 26px 24px; min-height: 100vh; background: #f7edf0; }
        .page-shell { max-width: 940px; }
        .alert-success { background: var(--success-bg); color: var(--success-text); border: 1px solid var(--success-border); padding: 14px 16px; border-radius: 14px; margin-bottom: 20px; }

        @media (max-width: 900px) {
            .sidebar { display: none; }
            .content { margin-left: 0; padding: 78px 16px 20px; }
